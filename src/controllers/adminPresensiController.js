// src/controllers/adminPresensiController.js
...
const getStatistikHarian = async (req, res) => {
  try {
    ...
    let totalUsersQuery = `SELECT COUNT(*) as total FROM users WHERE is_active = 1 AND roles = 'pegawai'`;
...
const getStatistikBulanan = async (req, res) => {
  try {
    ...
    let totalPegawaiQuery = `SELECT COUNT(*) as total FROM users WHERE is_active = 1 AND roles = 'pegawai'`;
    ...
    res.json({
      ...
      data: {
        bulan: targetBulan,
        tahun: targetTahun,
        nama_bulan: monthNames[targetBulan - 1],
...
const getPresensiBulanan = async (req, res) => {
  try {
    ...
    const summary = {
      total: result.length,
      hadir: result.filter(p => p.jam_masuk !== null && !p.izin_id).length,
      terlambat: result.filter(p => p.status_masuk === 'Terlambat' || p.status_masuk === 'Terlambat Berat').length,
      tepat_waktu: result.filter(p => p.status_masuk === 'Tepat Waktu').length,
      tanpa_keterangan: result.filter(p => p.status_masuk === 'Tanpa Keterangan' && !p.izin_id).length,
      izin: result.filter(p => p.izin_id !== null).length,
...
const getRekapKehadiranBulanan = async (req, res) => {
  try {
    ...
    presensiList.forEach(presensi => {
      ...
      const tanggalStr = presensi.tanggal instanceof Date 
        ? presensi.tanggal.toISOString().split('T')[0]
        : new Date(presensi.tanggal).toISOString().split('T')[0];
...
const exportRekapKehadiranExcel = async (req, res) => {
  try {
    ...
    presensiList.forEach(p => {
      ...
      const tanggalStr = p.tanggal instanceof Date 
        ? p.tanggal.toISOString().split('T')[0]
