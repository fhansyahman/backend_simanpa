...
/**
 * Fungsi untuk cek dan generate presensi untuk tanggal tertentu
 * REVISI: Menggunakan penugasan (bukan jam_kerja_id)
 */
const generatePresensiForTargetDate = async (targetDate) => {
  try {
    ...
    // HAPUS u.jam_kerja_id - Get semua user aktif
    ...
    for (const user of users) {
      try {
        ...
        if (existingPresensi.length > 0) {
          // Jika sudah ada, update penugasan jika perlu
          if (penugasan) {
            await pool.execute(
              `UPDATE presensi SET 
                penugasan_id = ?,
                is_penugasan_khusus = ?,
                updated_at = NOW()
               WHERE user_id = ? AND tanggal = ? AND (penugasan_id IS NULL OR penugasan_id != ?)`,
              [penugasan.id, penugasan.is_penugasan_khusus ? 1 : 0, user.id, targetDate, penugasan.id]
            );
          }
          skippedCount++;
        ...
        if (izin.length > 0) {
          ...
          // Buat presensi dengan izin DAN penugasan
          await pool.execute(
          ...
          if (defaultStatus) {
            ...
            // Fallback: Insert tanpa status_masuk
            await pool.execute(
              `INSERT INTO presensi 
        ...
        // Coba dengan metode fallback minimal
        try {
