...
const IzinService = {
  // Get all izin
  getAllIzin: async (filters) => {
    const { status, user_id, jenis, tanggal } = filters;
    const { query, params } = IzinModel.getAllIzinQuery(status, user_id, jenis, tanggal);
  ...
  getIzinById: async (id) => {
    const { query, params } = IzinModel.getIzinByIdQuery(id);
    const [izin] = await pool.execute(query, params);
    return izin;
  },

  // Get my izin
  getMyIzin: async (userId) => {
    const { query, params } = IzinModel.getMyIzinQuery(userId);
    const [izin] = await pool.execute(query, params);
    return izin;
  },

  // CREATE IZIN (USER BIASA) - SAMA PERSIS DENGAN KODE ASLI
  createIzin: async (userId, body, dokumen_pendukung) => {
    const { 
      jenis, 
      tanggal_mulai, 
      tanggal_selesai, 
    ...
    if (!jenis || !tanggal_mulai || !tanggal_selesai) {
      throw new Error('Jenis, tanggal mulai, dan tanggal selesai wajib diisi');
    }
    ...
    if (!validJenis.includes(jenis)) {
      throw new Error(`Jenis izin tidak valid. Pilihan: ${validJenis.join(', ')}`);
    }
  ...
  // UPDATE IZIN STATUS - SAMA PERSIS DENGAN KODE ASLI
  updateIzinStatus: async (id, status, adminId) => {
    ...
    if (status === 'Disetujui') {
      ...
      try {
        ...
        presensiGenerated = await IzinService.generatePresensiIzinSimpleWithLuxon(izin[0]);
  ...
  // FUNGSI GENERATE PRESENSI YANG SAMA PERSIS DENGAN KODE ASLI
  generatePresensiIzinSimpleWithLuxon: async (izin) => {
  ...
  // DELETE IZIN - SAMA PERSIS DENGAN KODE ASLI
  deleteIzin: async (id, userId) => {
    // Cek apakah izin exists
    const [izin] = await pool.execute(
    ...
    if (izin[0].status !== 'Pending') {
      throw new Error('Hanya bisa menghapus izin dengan status Pending');
    }
  ...
  // ADMIN CREATE IZIN - SAMA PERSIS DENGAN KODE ASLI
  adminCreateIzin: async (adminId, body) => {
    const {
      user_id,
      tanggal_mulai,
      tanggal_selesai,
    ...
    if (!user_id || !tanggal_mulai || !tanggal_selesai || !jenis) {
      throw new Error('User ID, tanggal mulai, tanggal selesai, dan jenis izin wajib diisi');
    }
    ...
    if (existingIzin.length > 0) {
      ...
      throw new Error(`User sudah memiliki izin yang disetujui pada tanggal ${existStart} s/d ${existEnd}`);
    }
    ...
    if (dokumen_pendukung) {
      try {
        ...
        if (dokumen_pendukung.startsWith('data:application/pdf;base64,')) {
          ...
          console.log('Menggunakan dokumen yang sudah ada:', dokumenFileName);
        }
      } catch (fileError) {
    ...
    // ============ PERBAIKAN UTAMA ============
    // Generate presensi otomatis jika status Disetujui
    if (status === 'Disetujui') {
      console.log('=== MEMULAI GENERATE PRESENSI OTOMATIS ===');
      
      try {
        // Siapkan data izin yang sudah di-insert
        const izinData = {
          id: result.insertId,
          user_id: user_id,
          tanggal_mulai: tanggalMulaiDB,
        ...
        console.log('Data izin untuk generate presensi:', izinData);
        
        // Gunakan fungsi generate yang sama seperti user biasa
        // Fungsi ini akan menghasilkan presensi untuk setiap hari dalam rentang tanggal
        const generatePresensiForIzin = async (izin) => {
          let generatedCount = 0;
          
          try {
            console.log('=== GENERATE PRESENSI IZIN ===');
            console.log('Izin ID:', izin.id, 'User ID:', izin.user_id);
            console.log('Tanggal mulai:', izin.tanggal_mulai);
            console.log('Tanggal selesai:', izin.tanggal_selesai);
            
            // Parse tanggal dengan benar
            let startDate = DateTime.fromISO(izin.tanggal_mulai, { zone: 'Asia/Jakarta' });
            let endDate = DateTime.fromISO(izin.tanggal_selesai, { zone: 'Asia/Jakarta' });
            
            if (!startDate.isValid || !endDate.isValid) {
              // Fallback ke SQL format
              startDate = DateTime.fromSQL(izin.tanggal_mulai, { zone: 'Asia/Jakarta' });
              endDate = DateTime.fromSQL(izin.tanggal_selesai, { zone: 'Asia/Jakarta' });
            }
            ...
            console.log('Start date parsed:', startDate.toFormat('dd/MM/yyyy'));
            console.log('End date parsed:', endDate.toFormat('dd/MM/yyyy'));
            ...
            while (currentDate <= endDate) {
              dayCounter++;
              const tanggal = currentDate.toFormat('yyyy-MM-dd');
              console.log(`Processing day ${dayCounter}: ${tanggal}`);
              
              try {
                // Cek apakah sudah ada presensi untuk tanggal ini
                const [existingPresensi] = await pool.execute(
                  'SELECT id, status_masuk, status_pulang FROM presensi WHERE user_id = ? AND tanggal = ?',
                  [izin.user_id, tanggal]
                );
                
                const statusIzin = 'Izin'; // Status untuk izin
                const statusPulang = 'Izin'; // Status pulang untuk izin
                const keteranganIzin = `Izin ${izin.jenis} - Auto generated by admin`;
                ...
                if (existingPresensi.length === 0) {
                  ...
                  const [insertResult] = await pool.execute(
                    `INSERT INTO presensi 
                     (user_id, izin_id, tanggal, status_masuk, status_pulang, 
                      is_system_generated, keterangan, created_at, updated_at) 
                     VALUES (?, ?, ?, ?, ?, 1, ?, NOW(), NOW())`,
                    [
                      izin.user_id,
                      izin.id,
                  ...
                  console.log(`✓ Inserted new presensi ID: ${insertResult.insertId} for ${tanggal}`);
                  generatedCount++;
                } else {
                  ...
                  const [updateResult] = await pool.execute(
                    `UPDATE presensi SET 
                      ...
                      status_masuk = ?, 
                      status_pulang = ?,
                      keterangan = CONCAT(?, ' (Updated by admin)'),
                  ...
                  console.log(`✓ Updated existing presensi for ${tanggal}, rows affected: ${updateResult.affectedRows}`);
                  generatedCount++;
                }
              } catch (error) {
                console.error(`✗ Error for date ${tanggal}:`, error.message);
            ...
            console.error('!!! ERROR in generatePresensiForIzin:', error);
            throw error;
          }
        ...
        // Panggil fungsi generate presensi
        presensiGenerated = await generatePresensiForIzin(izinData);
        
        console.log(`✅ Generate presensi selesai: ${presensiGenerated} record`);
        
      } catch (presensiError) {
        console.error('❌ Presensi generation failed:', presensiError);
        console.error('Error details:', presensiError.stack);
        presensiGenerated = 0;
      }
    }

    // Log activity
    const logDescription = `Admin ${adminId} membuat izin ${jenis} untuk ${user[0].nama} selama ${durasiHari} hari`;
  ...
  // GET MY IZIN PER BULAN - SAMA PERSIS DENGAN KODE ASLI
  getMyIzinPerBulan: async (userId, bulan, tahun) => {
    // Ambil parameter bulan dan tahun dari query string
    let { bulan: bulanParam, tahun: tahunParam } = { bulan, tahun };
    
    console.log('Raw params - Bulan:', bulanParam, 'Tahun:', tahunParam);
    ...
    let targetBulan;
    if (bulanParam && bulanParam !== '' && !isNaN(parseInt(bulanParam))) {
      targetBulan = parseInt(bulanParam);
    } else {
    ...
    let targetTahun;
    if (tahunParam && tahunParam !== '' && !isNaN(parseInt(tahunParam))) {
      targetTahun = parseInt(tahunParam);
    } else {
  ...
  // GET IZIN PER TANGGAL (UNTUK ADMIN) - SAMA PERSIS DENGAN KODE ASLI
  getIzinPerTanggal: async (userRoles, tanggal, status, jenis, search) => {
    ...
    let { tanggal: tanggalParam, status: statusParam, jenis: jenisParam, search: searchParam } = 
      { tanggal, status, jenis, search };
    
    console.log('📊 Get Izin Per Tanggal - Params:', { tanggal: tanggalParam, status: statusParam, jenis: jenisParam, search: searchParam });
    
    // Default ke hari ini jika tidak ada tanggal
    let targetDate;
    if (tanggalParam && tanggalParam !== '') {
      targetDate = DateTime.fromISO(tanggalParam);
    ...
    return {
      ...
      filters: {
        ...
        jenis: jenisParam || null,
        search: searchParam || null
      }
    };
  },

  // GET IZIN TANGGAL OPTIONS - SAMA PERSIS DENGAN KODE ASLI
  getIzinTanggalOptions: async (userRoles) => {
    // Hanya admin dan atasan yang bisa akses
    if (userRoles !== 'admin' && userRoles !== 'atasan') {
      throw new Error('Akses ditolak. Hanya admin dan atasan yang dapat mengakses.');
    }
  ...
  // DOWNLOAD DOKUMEN - SAMA PERSIS DENGAN KODE ASLI
  downloadDokumen: async (userRoles, filename) => {
    // Validasi akses (hanya admin dan atasan)
    if (userRoles !== 'admin' && userRoles !== 'atasan') {
      throw new Error('Akses ditolak. Hanya admin dan atasan yang dapat mengunduh dokumen.');
    }
    ...
    if (!filename.toLowerCase().endsWith('.pdf')) {
      throw new Error('Hanya file PDF yang dapat diunduh');
    }
  ...
  // CHECK IZIN BEFORE PRESENSI - SAMA PERSIS DENGAN KODE ASLI
  checkIzinBeforePresensi: async (userId, tanggal) => {
    const [izin] = await pool.execute(
      `SELECT * FROM izin 
