...
const checkHariKerja = async (tanggal) => {
  try {
    const hariKerja = await PresensiModel.getHariKerjaByTanggal(tanggal);
    ...
    const hariLibur = await PresensiModel.getHariLiburByTanggal(tanggal);
...
const checkUserIzin = async (userId, tanggal) => {
  try {
    const izin = await PresensiModel.getUserIzinByTanggal(userId, tanggal);
...
const generatePresensiForDate = async (targetDate) => {
  try {
    ...
    const users = await PresensiModel.getAllActiveUsers();
    ...
    for (const user of users) {
      ...
      try {
        ...
        const existingPresensi = await PresensiModel.checkExistingPresensi(user.id, targetDate);
        ...
        if (existingPresensi.length > 0) {
          ...
          if (izin && !presensi.izin_id) {
            ...
            const keterangan = presensi.keterangan 
              ? `${presensi.keterangan} | Auto-updated: Izin ${izin.jenis}`
              : `Auto-updated: Izin ${izin.jenis}`;
            
            await PresensiModel.updatePresensiWithIzin(izin.id, statusIzin, keterangan, presensi.id);
            updatedCount++;
            izinCount++;
            actionTaken = true;
        ...
        if (izin) {
          ...
          await PresensiModel.insertPresensiWithIzin(
            user.id, targetDate, penugasan ? penugasan.id : null,
            izin.id, statusIzin, `Auto-generated: Izin ${izin.jenis}`
          );
          generatedCount++;
          izinCount++;
          console.log(`  ✅ Created with izin: ${izin.jenis}`);
        } else {
          await PresensiModel.insertPresensiNoStatus(
            user.id, targetDate, penugasan ? penugasan.id : null,
            penugasan ? penugasan.is_penugasan_khusus : false
          );
          generatedCount++;
    ...
    await PresensiModel.insertSystemLog('GENERATE_PRESENSI', `Generated presensi ${targetDate}: ${generatedCount} new, ${updatedCount} updated, ${izinCount} izin, ${skippedCount} skipped`, null);
...
const generatePresensiHariIniOnStartup = async () => {
  try {
    ...
    const count = await PresensiModel.getCountPresensiForDate(today);
...
const checkAndUpdateIzinPresensi = async (tanggal) => {
  try {
    ...
    const usersWithIzin = await PresensiModel.getUsersWithIzin(tanggal);
    ...
    for (const user of usersWithIzin) {
      try {
        ...
        const existingPresensi = await PresensiModel.getPresensiByUserAndDate(user.id, tanggal);
        ...
        if (existingPresensi.length > 0) {
          ...
          if (needUpdate) {
            ...
            await global.pool.execute(query, updateValues);
            updatedCount++;
            console.log(`  🔄 Updated ${user.nama}: ${user.jenis}`);
          }
        } else {
          await PresensiModel.insertPresensiWithIzin(
            user.id, tanggal, penugasan ? penugasan.id : null,
            user.izin_id, statusIzin, `Auto: Izin ${user.jenis}`
          );
          createdCount++;
...
const generatePresensiForDateRange = async (startDate, endDate) => {
  try {
    ...
    await PresensiModel.insertSystemLog('GENERATE_PRESENSI_RANGE', `Generated presensi for range ${startDate} to ${endDate}: ${successCount} success, ${failedCount} failed`, null);
...
const updateTanpaKeteranganEarly = async () => {
  try {
    ...
    const [presensiList] = await global.pool.execute(
    ...
    await PresensiModel.insertSystemLog('UPDATE_TANPA_KETERANGAN_EARLY', `Early update at 20:00: ${updatedCount} records marked as Tanpa Keterangan`, null);
...
const presensiMasuk = async (req, res) => {
  try {
    ...
    const existingPresensi = await PresensiModel.checkExistingPresensi(userId, today);

    if (existingPresensi.length === 0) {
      console.log('⚠️ No presensi record found for today, creating one...');
      await generatePresensiForDate(today);
      
      const newPresensi = await PresensiModel.checkExistingPresensi(userId, today);
    ...
    await PresensiModel.updatePresensiMasuk(
      penugasan.id,
      penugasan.is_penugasan_khusus,
    ...
    await PresensiModel.insertSystemLog('PRESENSI_MASUK', `User melakukan presensi masuk - Status: ${statusMasuk} - Penugasan: ${penugasan.nama_penugasan}`, userId);
...
const presensiPulang = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getPresensiForPulang(userId, today);
    ...
    await PresensiModel.updatePresensiPulang(
      now.toFormat('HH:mm:ss'),
      fotoFileName,
    ...
    await PresensiModel.insertSystemLog('PRESENSI_PULANG', `User melakukan presensi pulang - Status: ${statusPulang}`, userId);
...
const getPresensiHariIni = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getPresensiTodayWithJoins(userId, today);
...
const getPresensiUser = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getAllPresensiUser(userId);
...
const getPresensiUserPerBulan = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getPresensiUserPerBulan(userId, startDate, endDate);
    ...
    const availableYearsData = await PresensiModel.getAvailableYearsPresensi(userId);
...
const getAllPresensi = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getAllPresensiAdmin(user_id, tanggal, wilayah, status_masuk, limit, offset);

    const totalCount = await PresensiModel.countAllPresensiAdmin(user_id, tanggal, wilayah, status_masuk);
...
const getAllPresensiPerBulan = async (req, res) => {
  try {
    ...
    const presensi = await PresensiModel.getAllPresensiPerBulanAdmin(startDate, endDate, wilayah);
    ...
    const pegawaiList = await PresensiModel.getAllActivePegawai(wilayah);
...
const getRekapPresensi = async (req, res) => {
  try {
    ...
    const rekap = await PresensiModel.getRekapPresensi(startDate, endDate, wilayah);
...
const generatePresensiManual = async (req, res) => {
  try {
    ...
    await PresensiModel.insertSystemLog('ADMIN_GENERATE_PRESENSI', `Admin generate presensi untuk ${targetDate}`, req.user.id);
...
const generatePresensiHariIni = async (req, res) => {
  try {
    ...
    await PresensiModel.insertSystemLog('EMERGENCY_GENERATE', `Emergency generate presensi hari ini ${today}`, req.user?.id || 1);
...
const generatePresensiRange = async (req, res) => {
  try {
    ...
    await PresensiModel.insertSystemLog('ADMIN_GENERATE_PRESENSI_RANGE', `Admin generate presensi untuk range ${start_date} hingga ${end_date}`, req.user.id);
...
const getSystemStatus = async (req, res) => {
  try {
    ...
    const todayCount = await PresensiModel.getCountPresensiForDate(today);
    const tomorrowCount = await PresensiModel.getCountPresensiForDate(tomorrow);
    const yesterdayCount = await PresensiModel.getCountPresensiForDate(yesterday);
    
    const activeUsers = await PresensiModel.getActiveUsersCount();
    
    const lastCron = await PresensiModel.getLastCronLog();
    
    const hariKerjaInfo = await checkHariKerja(today);
    
    const todayStats = await PresensiModel.getTodayStats(today);
...
const fixPresensiData = async (req, res) => {
  try {
    ...
    const presensiList = await PresensiModel.getPresensiForRangeFix(start_date, end_date);
    ...
    for (const presensi of presensiList) {
      try {
        ...
        if (izin && !presensi.izin_id) {
          const statusIzin = `Izin ${izin.jenis}`;
          const keterangan = `CONCAT(COALESCE(keterangan, ''), ' - Fixed: Izin ${izin.jenis}')`;
          
          await PresensiModel.fixPresensiWithIzin(izin.id, statusIzin, keterangan, presensi.id);
          fixedCount++;
          ...
          await PresensiModel.fixPresensiRemoveIzin(presensi.id);
          fixedCount++;
    ...
    await PresensiModel.insertSystemLog('FIX_PRESENSI_DATA', `Fixed ${fixedCount} presensi records, ${skippedCount} skipped untuk periode ${start_date} hingga ${end_date}`, req.user?.id || 1);
...
const generatePresensiOtomatis = async (req, res) => {
  try {
    ...
    await PresensiModel.insertSystemLog('GENERATE_PRESENSI_OTOMATIS', `System generate presensi otomatis untuk ${today}`, req.user?.id || 1);
...
const getGenerateStats = async (req, res) => {
  try {
    ...
    const generateStats = await PresensiModel.getGenerateStats(startDate, endDate);
    const hariKerjaStats = await PresensiModel.getHariKerjaStats(startDate, endDate);
    const generateLogs = await PresensiModel.getGenerateLogs(startDate, endDate);
...
const getDashboardKehadiran = async (req, res) => {
  try {
    ...
    const allPegawai = await PresensiModel.getAllPegawaiForDashboard();

    const totalPegawai = allPegawai.length;
    console.log(`Total pegawai aktif: ${totalPegawai}`);

    const presensiHariIni = await PresensiModel.getPresensiWithUserForDate(targetDateStr);
    const izinAktif = await PresensiModel.getActiveIzinForDateDetailed(targetDateStr);
    const kinerjaHariIni = await PresensiModel.getKinerjaForDateDetailed(targetDateStr);
      ...
      data: {
        ...
        charts: {
          kehadiran: {
            ...
            datasets: [{
              data: [
                sudahHadirDanLapor.length + belumLapor.length,
                izinList.length,
                belumHadir.length
              ],
...
const getDaftarBelumAbsen = async (req, res) => {
  try {
    ...
    const [allPegawai] = await global.pool.execute(pegawaiQuery, params);
    
    const presensiHariIni = await PresensiModel.getPresensiByDateOnly(targetDateStr);
    const izinAktif = await PresensiModel.getActiveIzinSimple(targetDateStr);
